...
class MLP_INFERENCE_CONTEXT:
    ...
    def do_inference(self,batch_data = []):
        ...
        print("dataset 加载成功")
        model=model_infer.BERT_Classifier(7)
...
if __name__ == '__main__':
    config={}
    config["batchsize"]=1
    MLP=MLP_INFERENCE_CONTEXT(config)
    batch_data=[{"reviewid":1,"sub":'家居用品',"obj":'裤架',"sub_type":"物","obj_type":"物",
                  "reviewbody":"宜家布梅朗裤架是一款非常实用的家居用品"}]
    print(MLP.do_inference(batch_data))
