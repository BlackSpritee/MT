...
class GCN(nn.Module):
    def __init__(self,nfeature,nhidden,nclass,args):
        super(GCN, self).__init__()
        self.gcn1=GCNConv(nfeature,nhidden)
        self.init_rel = get_param((32, nfeature)) #边的类别：节点类别-节点类别
...
def load_graph_data(args):
    ...
    if args.init_mode=="random":
        ...
        print("随机初始化向量")
    return edges, tuple_data ,features,ent2id,edges_type
...
if __name__ == '__main__':
    ...
    # 载入数据
    edges, tuple_data,features,ent2id,edges_type= load_graph_data(args)
    # Model and optimizer
