...
BERT_PATH='/home/hadoop-aipnlp/dolphinfs/hdd_pool/data/baichuanyang/bert-base-chinese/'
...
class GCN(nn.Module):
    ...
    def forward(self,index,label,sentence_mask,features):
        ...
        x = self.take_logtis(logits_gcn,index)
        x= torch.cat(x,dim=1)
        x = self.fc(x)
...
class SenGraph(nn.Module):
    def __init__(self,nfeature,nhidden,num_class,dropout):
        ...
        # self.e1_e2_fc=nn.Linear(nfeature*2,nhidden)
        self.class_fc=nn.Linear(nhidden,num_class)
        self.criterion = nn.CrossEntropyLoss()

    def forward(self,index,label,sentence_mask,features):
        # edges=torch.stack(index,dim=0)
        # logits_gcn=self.gcn(features,edges)
        # logits_gcn=F.relu(logits_gcn)
        # e1_e2=self.take_logtis(logits_gcn,index) #bacth*2*768
        sentence_encode=self.bert(sentence_mask[:,0,:],sentence_mask[:,1,:])[0][:,0,:] #cls
        # logits_sentence=self.bert_fc(sentence_encode)
        # logits=torch.cat([sentence_encode,e1_e2[0],e1_e2[1],e1_e2[1]-e1_e2[0]],dim=1)
        output = self.class_fc(sentence_encode)
...
def map_id_rel():
    rel2id = {'hypernym':0, 'synonym':1, 'needa':2, 'uncorrelated':3, 'needspecific':4, 'correlated':5, 'hasa':6}
    id2rel = {}
    for i in rel2id:
        id2rel[rel2id[i]] = i
...
def load_graph_data(args):
    ...
    for line in open(args.graph_path):
        sub, obj, rel,_,_ = map(str.lower, line.strip().split('\t'))
        ent_set.add(sub)
        rel_set.add(rel)
        ent_set.add(obj)
    ...
    tuple_data=[[],[],[],[]]
    for line in open(args.graph_path):
        sub, obj, rel,sentence,_ = map(str.lower, line.strip().split('\t'))
    ...
    if args.init_mode=="random":
        ...
        print("随机初始化向量")
    return edges, tuple_data ,features,ent2id,rel2id
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_path', default="ugc_weight_result.txt")
    parser.add_argument('--emb_path', default="/home/hadoop-aipnlp/cephfs/data/baichuanyang/project/Rert_re/mt_data/5e1$e2_768_model.pt")
    ...
    # 载入数据
    edges, tuple_data,features,ent2id,rel2id= load_graph_data(args)
    # Model and optimizer
