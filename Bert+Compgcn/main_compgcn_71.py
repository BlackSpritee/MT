...
rel2id = {'synonym': 0, 'hypernym': 1, 'uncorrected': 2, 'needa': 3, 'needspecific': 4, 'correlated': 5,
          'hasa': 6,'typical':7,'suitable':8}
def get_param(shape):
...
class GCN(nn.Module):
    def __init__(self,nfeature,nhidden,nclass,n_edge_type,args):
        ...
        self.fc=nn.Linear(nhidden*2,nhidden)
    def forward(self,index,edges_type,features,edges):
        ...
        x,r=self.compgcn(x,edges,edges_type,r)
...
class SenGraph(nn.Module):
    def __init__(self,nfeature,nhidden,nclass,n_edge_type,args):
        ...
        self.gcn=GCN(nfeature,nhidden,nclass,n_edge_type,args)
        self.bert=BertModel.from_pretrained('/home/hadoop-aipnlp/dolphinfs/hdd_pool/data/baichuanyang/bert-base-chinese/')
        self.class_fc=nn.Linear(nhidden*2,nclass)
        self.criterion = nn.CrossEntropyLoss()

    def forward(self,index,label,sentence_mask,edges_type,features,edges):
        logits_gcn=self.gcn(index,edges_type,features,edges)
        sentence_encode=self.bert(sentence_mask[:,0,:],sentence_mask[:,1,:])[0][:,0,:] #cls
        logits=torch.cat([logits_gcn,sentence_encode],dim=1)
...
def load_graph_data(args):
    ...
    # for line in open("cross_24w.txt"):
    ...
    for line in open(args.predict_data):
        reviewid,sub, obj,sub_type, obj_type,reviewbody = map(str.lower, line.strip().split('\t'))
        edge_type_set.add(sub_type+obj_type)
        edge_type_set.add(obj_type+sub_type)
    ...
    class_count={'synonym': 0, 'hypernym': 0, 'uncorrelated': 0, 'needa': 0, 'needspecific': 0, 'correlated': 0, 'hasa': 0}
    tuple_data=[[],[],[],[],[]]
    for line in open("cross_24w.txt"):
        sub,sub_type, obj, obj_type,rel,_,_,_ = map(str.lower, line.strip().split('\t'))
        if rel=="uncorrelated":
    ...
    for line in open(args.graph_path):
        sub,sub_type, obj, obj_type,rel,sentence = map(str.lower, line.strip().split('\t'))
    ...
    for line in open(args.predict_data):
        ...
        if i>5000:
            break
        sub, sub_type, obj, obj_type,sentence = map(str.lower, line.strip().split('\t'))
    ...
    # for line in open(args.graph_path):
    #     sub,sub_type, obj, obj_type,rel,_,_,_ = map(str.lower, line.strip().split('\t'))
    #     edges.append([ent2id[obj], ent2id[sub]])
    #     edges_type.append(reltype2id[obj_type+sub_type])
    # for line in open(args.predict_data):
    #     sub, sub_type, obj, obj_type,_ = map(str.lower, line.strip().split('\t'))
    #     edges.append([ent2id[obj], ent2id[sub]])
    #     edges_type.append(reltype2id[obj_type+sub_type])

    print("边的个数",len(edges))
    edges=torch.tensor(np.array(edges,dtype="int32"))
    edges_type=torch.tensor(np.array(edges_type,dtype="int32"))
    ...
    if args.init_mode=="random":
        ...
        print("随机初始化向量")
    return edges, tuple_data ,features,ent2id,edges_type,len(reltype2id),predict_data,class_count
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_path', default="ugc_new.txt")
    parser.add_argument('--emb_path', default="/home/hadoop-aipnlp/dolphinfs/hdd_pool/data/baichuanyang/project/verniesage3/verniesage2/cora/2/pytorch_model.bin")
    parser.add_argument('--cuda', action='store_true', default=True,
                        help='Disables CUDA training.')
    # parser.add_argument('--predict_data', action='store_true', default="predict_ingraph.txt")
    parser.add_argument('--predict_data', action='store_true', default="predict.txt")
    ...
    # 载入数据
    edges, tuple_data,features,ent2id,edges_type,len_reltype,predict_data,class_count= load_graph_data(args)
    # Model and optimizer
    ...
    def train(args):
        ...
        for e1_index,e2_index,label,sentence_mask,_ in train_iter:
            if flag<=1:
                print(label.detach().cpu().numpy().tolist())
                flag+=1
            else:
                break
        for epoch in range (args.num_epoch):
            ...
            # if epoch%2==1:
            #     torch.save({'state_dict': {'network': model.state_dict(), 'optimizer': optimizer.state_dict()},
            #                 'epoch': epoch}, str(epoch) + '_24w_ugcbert_corr_compgcn521.pt')
            if epoch>5:
                predict(args,model,predict_data,epoch)
    ...
    def eval(args,model):
        ...
        # with open("result_eval_ugcbert_24w_pre+cross","w",encoding='utf8') as w:
        #     for line in result:
        #         for i in line:
        #             w.write(i+'\t')
        #         w.write('\n')
    ...
    def predict(args,model,predict_data,epoch):
        predict_turple_data=copy.deepcopy(predict_data)[:5000]
        ...
        for i in ent2id:
            id2ent[ent2id[i]] = i
        model = model.eval()
        predict_iter = torch.utils.data.DataLoader(predict_dataset, args.batch_size, shuffle=False)
        with torch.no_grad():
            for e1_index, e2_index, label, sentence_mask, _ in predict_iter:
                loss, output = model([e1_index, e2_index], label, sentence_mask.squeeze(2), edges_type,
                                     features, edges)
                ...
                for i in range(len(label)):
                    result.append([id2ent[e1_index.cpu().numpy()[i]], id2ent[e2_index.cpu().numpy()[i]],
                                id2rel[predicted.cpu().numpy()[i]],
                                   score.cpu().numpy()[i]])
        with open("result_predict_24w_ugcbert_524_alledges_seng_newugc_weightsample_"+str(epoch), "w", encoding='utf8') as w:
            for line in result:
                # print(line)
                for i in line:
                    w.write(str(i) + '\t')
