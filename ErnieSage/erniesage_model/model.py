...
class ErnieSageForLinkPrediction(ErniePretrainedModel):
    ...
    def __init__(self, ernie, config):
        ...
        self.fc=paddle.nn.Linear(256,config.num_class)
    def forward(self, graphs,term_ids, data):
        ...
        """
        inputs_e1 = paddle.to_tensor(np.array(data)[:, 0])
        inputs_e2 = paddle.to_tensor(np.array(data)[:, 1])
        real_label = paddle.to_tensor(np.array(data)[:, 2].reshape([len(np.array(data)[:, 0]), 1]))
        # encoder model
        outputs = self.encoder([graphs], term_ids,
                               [inputs_e1, inputs_e2])
        logits = paddle.concat(outputs, axis=-1)
