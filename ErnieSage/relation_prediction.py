...
def dump_node_feat(config, entity_set,sentence_set):
    ...
    print("stat tokenize")
    tokenizer = ernie.tokenizing_ernie.ErnieTokenizer.from_pretrained(config.model_name_or_path)
    term_ids = [
        partial(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", type=str, default="./config/re_erniesage.yaml")
    ...
    ernie_cfg_dict, ernie_param_path = PretrainedModelLoader.from_pretrained(
        os.path.join(current_directory,config.model_name_or_path))
    ...
    if args.do_predict:
        ...
        do_train(config)
# python -m paddle.distributed.launch --gpus "1" relation_prediction.py --conf ./config/re_erniesage.yaml --do_predict
