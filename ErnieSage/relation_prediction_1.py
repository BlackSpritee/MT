...
def load_train_test_data(file_path, entity_map, rel_map):
    Dataset = namedtuple("Dataset",
                         ["e1_index", "e2_index", "label","sentence"])
    train_e1_index = []
    train_e2_index = []
    train_label = []
    sentence_list=[]
    for line in open(file_path):
        ...
        sentence_list.append(sub+'$'+obj+'$'+sentence)
    dataset = Dataset(
...
def dump_node_feat(config, entity_set,sentence_set):
    ...
    sentence_ids = [
        partial(
            term2id, tokenizer=tokenizer, max_seqlen=config.sentence_max_seqlen)(s)
        for s in sentence_set
    ]
    return term_ids, tokenizer, tokenizer.cls_id ,sentence_ids
...
def do_eval(model,config):
    ...
    node_feature, tokenizer, config.cls_id,sentence_ids = dump_node_feat(config, ent2id, infer_dataset[3])
    # print(node_feature)
    for i,sentence_id in enumerate(sentence_ids):
        infer_dataset[3][i]=sentence_id
    ...
    def infer_reader():
        for index in range(len(infer_dataset.label)):
            yield [infer_dataset.e1_index[index], infer_dataset.e2_index[index],
                   infer_dataset.label[index],infer_dataset.sentence[index]]

    infer_batch_dataset = paddle.batch(infer_reader, batch_size=32)
    model.eval()
    ...
    for step, infer_iter in  enumerate(infer_batch_dataset()):
        ...
        for i,o in enumerate(outputs):
            ...
            if list(pre_label)[0]==real_label[0]:
                true_prenum+=1
            num+=1
...
def do_train(config):
    ...
    node_feature, tokenizer,config.cls_id , sentence_ids = dump_node_feat(config, ent2id,train_dataset[3])
    for i,sentence_id in enumerate(sentence_ids):
        train_dataset[3][i]=sentence_id
    ...
    def train_reader():
        for index in range(len(train_dataset.label)):
            yield [train_dataset.e1_index[index], train_dataset.e2_index[index],
                   train_dataset.label[index],train_dataset.sentence[index]]
    ...
    ernie_sentence =  ErnieModel.from_pretrained("ernie-tiny")
    ernie_graph = ErnieModel.from_pretrained("ernie-tiny")
    model = ErnieSageForLinkPrediction(ernie_graph,ernie_sentence,config)
        ...
        for step, train_iter in enumerate(train_batch_dataset()):
            ...
            if global_step % config.save_per_step == 0:
                if rank == 0:
                    ...
                    paddle.save({'model':model.state_dict(),'opt':optimizer.state_dict(),'eopch':epoch},os.path.join(output_dir,'model.pdparams'))
    if rank == 0:
...
def do_predict(config):
    ...
    model_class, _ = MODEL_CLASSES[config.ernie_name]
    model_load=paddle.load(os.path.join(config.infer_model+'model.pdparams') )
    # enrie_gragh = ErnieModel().load_the_state(model)
    ernie_graph =  ErnieModel.from_pretrained("ernie-tiny")
    ernie_sentence =  ErnieModel.from_pretrained("ernie-tiny")
    model = ErnieSageForLinkPrediction(ernie_graph ,ernie_sentence,config)

    model.load_dict(model_load["model"])
    model.eval()
    model = paddle.DataParallel(model)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", type=str, default="./config/re_erniesage_1.yaml")
...
# python -m paddle.distributed.launch --gpus "2" relation_prediction_1.py --conf ./config/re_erniesage_1.yaml --do_predict
