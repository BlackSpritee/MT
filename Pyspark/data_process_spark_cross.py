...
if __name__ == '__main__':
    ...
    concept_dictset = get_dictset('cross_ugc')
    property_dict = get_dictset('cross_ugc')
    #----加载词表完成！
    ...
    def spark_data_process(rows):
        ...
        for line in lines_list:
            ...
            for index in range(len(line)-2):
                if line[index] in concept_dictset and line[index+1] in property_dict and line[index]!=line[index+1]:#应该做一个去重类别处理
                    if line[index] == '' or line[index]=='':
                        continue
                    result.append([u''.join(line) , line[index]+ u'_' + line[index+1], line[index], line[index+1]])
        return result
    cut_comment_table_name = 'mart_aikg.cross_cross_comment_table'
